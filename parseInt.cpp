...
bool WriteStmt(istream& in, int& line) {
    ...
    if (!ex) {
        //empty the ValQue and delete it
        while(!(*ValQue).empty()){
            ValQue->pop();
        }
        delete ValQue;
...
bool AssignStmt(istream& in, int& line) {
    ...
    Token type = SymTable[identifier];
    ...
    if (varstatus) {
        ...
        if (t == ASSOP) {
            ...
            if((type==FLOAT||type==INT)&&retVal.IsChar()){
                ParseError(line, "Illegal Assignment Operation");
                return false;
            }
            if(type==FLOAT&&retVal.IsInt()){
                TempsResults[identifier] = Value((int)retVal.GetInt());
            }else if(type==INT&&retVal.IsReal()){
                TempsResults[identifier] = Value((int)retVal.GetReal());
            }else {
                TempsResults[identifier] = retVal;
            }
        }
...
bool Expr(istream& in, int& line, Value& retVal) {
    ...
    while (tok == PLUS || tok == MINUS)
    {
        ...
        if(tok==PLUS){
            if ((val1.IsInt() || val1.IsReal()) && (val2.IsInt() || val2.IsReal())) {
                val1 = val1 + val2;
            }
            else {
                ParseError(line, "Illegal addition operand.");
                return false;
            }
        }
        ...
        if(tok==MINUS){
            if ((val1.IsInt() || val1.IsReal()) && (val2.IsInt() || val2.IsReal())) {
                ...
                ParseError(line, "Illegal subtraction operand.");
                return false;
            }
        }
